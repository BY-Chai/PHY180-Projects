...
def plot_fit(my_func, xdata, ydata, xerror=None, yerror=None, init_guess=None, font_size=14,
             xlabel="Independant Variable (units)", ylabel="Dependent Variable (units)", titlelabel="Pendulum Period over Q Factor"):
    ...
    ax1.errorbar(xdata, ydata, yerr=yerror, xerr=xerror, fmt=".", label="Data (Starting Amplitude = 0.349rad)", color="black", lw=1)
    ...
    ax1.plot(xs, curve, label=f"Best fit coeffs = {list(np.round(popt, 2))}", color="blue")
    ...
    ax1.set_title(label = titlelabel, fontweight = 30, pad = 2.0)
    # WHole graph title
